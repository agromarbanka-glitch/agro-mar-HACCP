...
async function allocateFifo(operationId, productId, qtyNeeded, operationDate) {
  let remainingToAllocate = Math.abs(Number(qtyNeeded) || 0)
  const allocations = []
  const saleDate = operationDate || new Date().toISOString().slice(0, 10)

  // FIFO nie może używać PZ z przyszłości względem WZ/FV.
  // Do rozchodu z dnia saleDate dopuszczamy tylko partie z datą PZ <= saleDate.
  const { data: lots, error } = await supabase
    .from('lots')
    .select('id, remaining_qty, production_date, created_at')
    .eq('product_id', productId)
    .lte('production_date', saleDate)
    .gt('remaining_qty', 0)
